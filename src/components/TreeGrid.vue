 ...
 * @events  @on-row-click 单击行或者单击操作按钮方法
 ...
 -->
<template>
    <div :style="{width:tableWidth, height: tableHeight}" class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table'>
            <thead>
                <tr>
                    <th v-for="(column,index) in cloneColumns" :key="column.id">
                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'action'">
                            <i-button :type="action.type" size="small" :icon="action.icon" style="margin-right:5px;" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)'  v-if="(item.children && action.text!='删除') || !item.children" :key="action.text">{{action.text}}</i-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <i v-if="item.children&&item.children.length>0" class="ivu-icon" style='width:14px;' :class="{'ivu-icon-arrow-right-b':!item.expanded,'ivu-icon-arrow-down-b':item.expanded }"></i>
